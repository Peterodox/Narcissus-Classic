...
local EquipmentManager_UnpackLocation = NarciClassicAPI.EquipmentManager.UnpackLocation;
...
function NarciEquipmentListFilterButtonMixin:OnLoad()
    ...
    --Classic: We disable this "Show Owned Item" toggle
    self:Hide();
end
...
function NarciEquipmentOptionMixin:SetGemListForBlizzardUI(id1, id2)
    ...
    self:SetPoint("TOPLEFT", parentFrame, "TOPRIGHT", 2, -12);  --Frame size is different from Retail
    self:Show();
    self:SetAlpha(1);
    self:ShowGemList();
...
function NarciEquipmentOptionMixin:Init()
    ...
    for i = 1, #BUTTON_DATA do
        ...
        button.type = BUTTON_DATA[i][1];
        button.Icon:SetTexture(BUTTON_DATA[i][2]);
        button.buttonName = BUTTON_DATA[i][3];
        button:SetButtonText(button.buttonName);
        button.disabledText = BUTTON_DATA[i][4];
...
function NarciEquipmentOptionMixin:ShowMenu()
    ...
    if false and validSlotForTempEnchants[self.slotID] then --Debug: Hide Temp Enchant on Classic
        numButtons = 4;
...
do
    ...
    if C_Container and C_Container.SocketContainerItem then
        hooksecurefunc(C_Container, "SocketContainerItem", function(bag, slot)
