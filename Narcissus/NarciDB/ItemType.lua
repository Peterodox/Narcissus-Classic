...
local function GetSlotNameBySlotID(slotID)
    if slotData[slotID] then
        return slotData[slotID][2]  --Finger1
    end
end

local function GetInventorySlotNameBySlotID(slotID)
    if slotData[slotID] then
        return slotData[slotID][1]  --Finger0Slot
    end
end
...
NarciAPI.GetSlotNameBySlotID = GetSlotNameBySlotID;
...
for key, data in pairs(itemTypes) do
    ...
    if itemID then
        ...
        --print("Item: "..tempItemID.." no longer exists");
    end
end
